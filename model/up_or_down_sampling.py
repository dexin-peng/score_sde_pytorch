...
def conv_downsample_2d(x, w, k=None, factor=2, gain=1):
  ...
  p = (k.shape[0] - factor) + (convW - 1)
  s = [factor, factor]
  x = upfirdn2d(x, k.to(x.device), pad=((p + 1) // 2, p // 2))
  return F.conv2d(x, w, stride=s, padding=0)


def _setup_kernel(k):
  k = torch.asarray(k, dtype=torch.float32)
  if k.ndim == 1:
    k = torch.outer(k, k)
  k /= torch.sum(k)
  assert k.ndim == 2
...
def upsample_2d(x, k=None, factor=2, gain=1):
  ...
  k = _setup_kernel(k) * (gain * (factor ** 2))
  p = k.shape[0] - factor
  return upfirdn2d(x, k.to(x.device), up=factor, pad=((p + 1) // 2 + factor - 1, p // 2))
...
def downsample_2d(x, k=None, factor=2, gain=1):
  ...
  k = _setup_kernel(k) * gain
  p = k.shape[0] - factor
  return upfirdn2d(x, k.to(x.device), down=factor, pad=((p + 1) // 2, p // 2))
